# Second version of BERTopic implementation for big data on HPC 
# Removed the 10-fold cross-validation for model selection
# replace the cluster method from KMeans to HDBSCAN
# Added the function to save the model and load the model
...
class BERTopicGPU(object):
    ...
    def finetune_bertopic(self, topics, probs, documents):
        ...
        topic_model.reduce_topics(documents, topics, probabilities=probs, nr_topics=20)

        # 3. Remove topics with low coherence (optional)
        coherence_scores = topic_model.topic_model_coherence(documents, topics)
        topic_model.update_topics(documents, topics, probabilities=probs, min_coherence=0.3)

        # 4. Find topic outliers and remove them (optional)
        outliers = topic_model.find_topic_outliers(topic_model.get_topic_freq(), z_threshold=2)
        topic_model.update_topics(documents, topics, probabilities=probs, topics_to_remove=outliers)

        # Get the final topics
        final_topics = topic_model.get_topic_info()
        return final_topics
